...
def calculate_real_option(params, kpi):
    ...
    if not (d < exp(r * T_opt) < u): return 0.0
    p = (exp(r * T_opt) - d) / (u - d)
...
def plot_tornado(df, base_npv, T):
    fig = go.Figure()
    fig.add_trace(go.Bar(y=df['variable'], x=df['npv_low'] - base_npv, base=base_npv, orientation='h', name='Low Case', marker_color='#E45756'))
    fig.add_trace(go.Bar(y=df['variable'], x=df['npv_high'] - base_npv, base=base_npv, orientation='h', name='High Case', marker_color='#54A24B'))
    fig.add_vline(x=base_npv, line_dash="dash", line_color="white", annotation_text="Base NPV")
    fig.update_layout(barmode='overlay', title_text=T['tornado_title'], xaxis_title="Net Present Value (NPV) in €", template="plotly_dark", height=400, legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1))
    return fig
...
def plot_heatmap(npv_matrix, x_values, y_values, T, var1_name, var2_name):
    fig = go.Figure(data=go.Heatmap(z=npv_matrix, x=x_values, y=y_values, colorscale='RdYlGn', zmid=0))
    fig.update_layout(title=T['heatmap_title'], xaxis_title=var1_name, yaxis_title=var2_name, template="plotly_dark", height=500)
    return fig
...
with st.sidebar.expander(T['capex_sidebar_header'], expanded=True):
    capex_items = {
        'dsm': st.number_input(T['capex_design'], value=422000, key='dsm', format="%d"),
        'pv': st.number_input(T['capex_pv'], value=2940000, key='pv', format="%d"),
        'bess': st.number_input(T['capex_battery'], value=2920000, key='bess', format="%d"),
        'elec': st.number_input(T['capex_electrolyzer'], value=1781000, key='elec', format="%d"),
        'aux': st.number_input(T['capex_auxiliary'], value=210000, key='aux', format="%d"),
        'civil': st.number_input(T['capex_civil'], value=854000, key='civil', format="%d")
    }
...
with st.expander(T['assumptions_header']):
    ...
    with col_b:
        st.subheader(T['capex_assumptions']); st.table(pd.DataFrame(list(zip([T['capex_design'], T['capex_pv'], T['capex_battery'], T['capex_electrolyzer'], T['capex_auxiliary'], T['capex_civil']], capex_items.values())), columns=[T['parameter'], T['value']]).style.format({T['value']: "€{:,.0f}"}))
    with col_c:
...
with tab_r3:
    ...
    # MODIFICATION: La liste des paramètres à suivre a été étendue pour plus de fiabilité
    # On inclut tous les inputs de la sidebar qui affectent le calcul
    current_params_tuple = (
        p_h2_price, p_grant_percent, p_discount_rate, p_inflation_rate, p_elec_price,
        p_replacement_year_stack, p_replacement_cost_stack, p_replacement_year_bess, p_replacement_cost_bess,
        *capex_items.values() # On ajoute tous les items du CAPEX
    )

    if 'last_params_mc' not in st.session_state or st.session_state.last_params_mc != current_params_tuple:
        with st.spinner("Simulation in progress... (this may take a moment)"):
            st.session_state.mc_results = run_monte_carlo(params)
            st.session_state.last_params_mc = current_params_tuple
